...
describe('Emoji Box Acceptance Tests', () => {
  ...
  beforeEach(async () => {
    // Navigate to a page with emoji functionality
  ...
  test('Clicking an emoji should add it to input', async () => {
    // This test only runs if there are emoji elements on the page
    ...
    if (emojiExists) {
      ...
      // Skip test if no emoji elements
      console.log('No emoji elements found, skipping test');
    }
  });
});
